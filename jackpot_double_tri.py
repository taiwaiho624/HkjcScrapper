...
class JackpotUrlGenerator(GenericUrlGenerator.GenericUrlGenerator):
    def Generate(self):            
        ...
        for i in range(1, 12):
            url = f'https://bet.hkjc.com/racing/pages/odds_dt.aspx?lang=en&date={race_day}&raceno={i}'

            dynamicFields = {
                "match_id" : race_day + str(i).zfill(2),
            }
...
class JackpotScrapper(GenericWebScrapper.GenericWebScrapper):
    def isValid(self, context):
        ...
        matchNo = str(context.soup.select('div[class*="raceNoOn_"]')[0])
        matchNo = int(matchNo[matchNo.find("(") + 1:matchNo.find(")")])

        if str(matchNo).zfill(2) != self.data["match_id"]["value"][-2:]:
            return False
    ...
    def decode(self, context):
        ...
        self.data["double_tri_jackpot"]["value"] = float(self.jackpot)
        self.data["match_id"]["value"] = util.ConstructMatchId(self.data["match_id"]["value"][:-2], a, venue)

if __name__ == "__main__":
    try: 
        tableName = "jackpot_double_tri"
